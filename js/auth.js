...
import { GoogleAuthProvider, signInWithPopup, signInWithCredential } from "https://www.gstatic.com/firebasejs/10.8.0/firebase-auth.js";
...
const processGoogleUser = async (user, errorDiv, btn, buttonLabel, googleSvg) => {
    try {
        ...
        if (errorDiv) errorDiv.innerText = error.message || 'An error occurred. Please try again.';
        if (btn) {
            btn.disabled = false;
...
const handleGoogleSignIn = async (btn, errorDiv, buttonLabel) => {
    ...
    // If running inside our Native Android App Wrapper
    if (window.AndroidAuth && typeof window.AndroidAuth.triggerNativeGoogleSignIn === 'function') {
        window.currentAndroidBtn = btn;
        window.currentAndroidErrorDiv = errorDiv;
        window.currentAndroidButtonLabel = buttonLabel;
        window.AndroidAuth.triggerNativeGoogleSignIn();
        return; // Android will call handleAndroidLoginSuccess/Error later
    }

    try {
        const result = await signInWithPopup(auth, googleProvider);
        await processGoogleUser(result.user, errorDiv, btn, buttonLabel, googleSvg);
    } catch (error) {
...
// Global handlers for Android Native Login response
window.handleAndroidLoginSuccess = async (idToken) => {
    try {
        const credential = GoogleAuthProvider.credential(idToken);
        const result = await signInWithCredential(auth, credential);
        await processGoogleUser(result.user, window.currentAndroidErrorDiv, window.currentAndroidBtn, window.currentAndroidButtonLabel, googleSvg);
    } catch (error) {
        console.error("Firebase Auth via Native Token failed", error);
        if (window.currentAndroidErrorDiv) window.currentAndroidErrorDiv.innerText = "Native sign-in failed: " + error.message;
        if (window.currentAndroidBtn) {
            window.currentAndroidBtn.disabled = false;
            window.currentAndroidBtn.innerHTML = googleSvg + ' ' + window.currentAndroidButtonLabel;
        }
    }
};

window.handleAndroidLoginError = (errorMsg) => {
    console.error("Android Native Auth Error:", errorMsg);
    if (window.currentAndroidErrorDiv) {
        // If the user just cancelled the dialog, don't show a big error
        if (errorMsg.includes("cancelled") || errorMsg.includes("user_canceled")) {
             window.currentAndroidErrorDiv.innerText = "";
        } else {
             window.currentAndroidErrorDiv.innerText = 'Native sign-in failed: ' + errorMsg;
        }
    }
    if (window.currentAndroidBtn) {
        window.currentAndroidBtn.disabled = false;
